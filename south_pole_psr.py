...
def img_corners_in_ring(filename:str, ring_dist:float, inner_ring_dist:float):
    corner_distances = filename_to_corner_distances(filename)
    return sum(map(lambda dist:dist <= ring_dist and dist > inner_ring_dist, corner_distances))
...
def cal(ring_num:int):
    ...
    inner_ring_dist = get_ring_dist(ring_num + 1) if ring_num + 1 != 90 else -1
    
    images_corners_in_ring =  map(lambda filename:img_corners_in_ring(filename, ring_dist, inner_ring_dist), filenames)
        ...
        with Image.open(f"{FILE_PATH}{search_img_name}") as img:
            if corners_in_ring == 4:
                ...
                for index, pixel in enumerate(img.getdata(0)):
                    ...
                    if pixel == MATCH_RED and pixel_dist <= ring_dist and pixel_dist > inner_ring_dist:
                        count += 1
